...
class Graph(BaseModel):
    ...
    def resolve_edges(self, sorted_nodes: List[Node]):
        ...
        for node in sorted_nodes:
            for edge in edge_lookup.get(node.id, []):
                ...
                match (src.data.variant, tar.data.variant):
                    ...
                    case ("function", "list"):
                        print("function -> list", flush=True)
                        src.protocol = "append"
                        tar.set_input_id(src)
                    case ("context", "list"):
                        src.add_default(tar)
    ...
    def precompile(
        self, sorted_nodes: List[Node], r_adj_list: Dict[str, List[str]]
    ) -> List[str]:
        code = []

        self.resolve_edges(sorted_nodes)

        for node in self.nodes:
            inputs = [self.lookup[input_id] for input_id in r_adj_list[node.id]]
